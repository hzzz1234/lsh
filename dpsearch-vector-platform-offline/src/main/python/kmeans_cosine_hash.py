...
def sigMatrixGen(input_matrix, cluster):
    """
    ...
    # result_pre = np.dot(cluster , input_matrix)
    # print result_pre
    result = np.where(np.dot(cluster , input_matrix)>0,1,0)
    # print result
    # return a list
    return result


def cosineHash(input_matrix, b, r,cluster):
    """
    ...
    for i in range(b):
        ...
        sigMatrix = sigMatrixGen(input_matrix, cluster[i*r:(1+i)*r])
...
def nn_search(dataSet, query):
    ...
    hashBucket = cosineHash(input_matrix, b, r,kmeans.cluster_centers_)
    print hashBucket
