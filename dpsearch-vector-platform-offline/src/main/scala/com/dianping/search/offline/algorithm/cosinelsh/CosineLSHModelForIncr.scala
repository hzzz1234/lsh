...
/**
  ...
  */
class CosineLSHModelForIncr(dimension : Int, numRows : Int,hash : List[(CosineHashVector, Int)]) extends Serializable {
...
//  final val hashVectors : List[(CosineHashVector, Int)] = _hashVectors.toList.zipWithIndex
  final val hashVectors : List[(CosineHashVector, Int)] = hash
...
//  var bands : RDD[((Int, Int), Iterable[String])] = null
  ...
  def filter(score : Double): CosineLSHModelForIncr = {
